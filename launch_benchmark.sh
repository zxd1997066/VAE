...
function main {
    ...
    do
        # pre run
        python test.py -b 1 --no-cuda --arch VAE --warmup-iterations 1 --num-iterations 2 ${CKPT_DIR} \
...
function generate_core {
    ...
    do
        ...
        printf " ${OOB_EXEC_HEADER} \
            python test.py --no-cuda --arch VAE ${CKPT_DIR} \
                --warmup-iterations ${num_warmup} --num-iterations ${num_iter} \
                -b ${batch_size} \
                --precision ${precision} \
...
function generate_core_launcher {
    ...
    do
        ...
        printf "python -m oob-common.launch --enable_jemalloc \
            ...
            test.py --no-cuda --arch VAE ${CKPT_DIR} \
                --warmup-iterations ${num_warmup} --num-iterations ${num_iter} \
                -b ${batch_size} \
                --precision ${precision} \
                --channels_last ${channels_last} \
